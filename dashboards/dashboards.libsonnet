  ...
  grafanaDashboards+:: {
    ...
    local interfaceSelector =
      ...
      dashboard.variable.query.refresh.onTime() +
        {
          local query = |||
              query_result(group(label_replace(ifAdminStatus{target=~"$target"}, "interfaceSelector", "$1", "ifName", "(.*)") or label_replace(ifAdminStatus{target=~"$target"}, "interfaceSelector", "$1", "ifAlias", "(.*)")) by (interfaceSelector))
            |||,
          query: query,
        },
